...
export function BudgetProgressBar({ budget }: Props) {
  ...
  const { totalBudget, spent, remaining, transport, hotel, activities } = budget;
  ...
  const transportPct = (transport / safeTotal) * 100;
    ...
    <Card className="mb-6 overflow-hidden">
      <CardContent className="p-5">
        ...
        <div className="h-3 w-full bg-slate-100 rounded-full overflow-hidden flex mt-4">
          {transport > 0 && (
            <div className="h-full bg-blue-500" style={{ width: `${transportPct}%` }} title={`Transport: ₹${transport}`} />
          )}
          {hotel > 0 && (
            <div className="h-full bg-indigo-500" style={{ width: `${hotelPct}%` }} title={`Hotel: ₹${hotel}`} />
          )}
          {activities > 0 && (
            <div className="h-full bg-teal-500" style={{ width: `${activitiesPct}%` }} title={`Activities: ₹${activities}`} />
          )}
          {isOverBudget && <div className="h-full bg-red-500 flex-1" />}
        </div>

        <div className="flex gap-4 mt-3 text-xs text-slate-500">
          <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-sm bg-blue-500" /> Transport</div>
